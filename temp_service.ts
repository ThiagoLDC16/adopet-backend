...
import type { Report, ReportListResponse, ReportFilters, CreateReportData, ReportStatus } from '../types/report.types';
...
export const reportService = {
  ...
  async updateReportStatus(id: number, status: ReportStatus): Promise<Report> {
    const response = await api.put(`/api/report/${id}/status`, { status });
    return response.data;
  }
};